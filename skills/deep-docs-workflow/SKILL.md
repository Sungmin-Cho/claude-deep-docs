...
4. For each authoring gap, call `authoring-baseline` before dispatching `doc-author`. Preserve unapproved removals, verify `preserved_blocks`, show the whole draft, and call `authoring-commit` only after approval. The host performs no second write or patch.
