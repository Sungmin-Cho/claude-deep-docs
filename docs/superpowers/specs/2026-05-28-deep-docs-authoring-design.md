...
| `skills/deep-docs/SKILL.md` | 변경 | garden authoring sub-flow, scan 리포트 authoring 집계, **scan "문서 없으면 종료" 개조** `[R3-plan-R4:🔴 entry-skill]` — entry skill 은 Codex/SDK self-contained 라 workflow SKILL 과 **별도로** "문서 0개 → missing-doc gap 탐지" 로 고쳐야 빈/신규 레포에서 gap emit |
...
- **출력 계약 (구조화 — 확정)** `[R1:F1,C5][R2: codex high default-keep]`: doc-author 는 파일을 쓰지 않고(Write·Bash 없음) **구조화된 result 객체를 반환**한다:
  ...
  ```
  **`[R3-plan-R4:🔴 base_hash 소유권]` TOCTOU baseline 은 garden 이 소유 (doc-author 아님)**: doc-author 는 `Read/Glob/Grep` 만이라 `git hash-object`(Bash) 를 계산할 수 없으므로 **`base_hash` 를 result 에 넣지 않는다**. 대신 **garden 이 baseline 을 직접 소유**한다 — garden 은 메인 세션이라 Bash 사용 가능.
  **garden 의 default-keep + TOCTOU 기계적 강제** (codex 2/3 — prose 가 아닌 contract 로): garden 은
  ① **TOCTOU baseline 캡처 + Write 직전 재확인 (garden 소유, create/restructure 양쪽)** `[R3-plan:🔴 create-TOCTOU][R3-plan-R4:🔴 base_hash]`:
     - **restructure**: garden 이 doc-author **spawn 전에** `baseline = git hash-object <target_path>` 를 보관 → Write **직전** `git hash-object <target_path>` 재계산해 baseline 과 비교, 불일치(scan~Write 사이 변경)면 fail-closed(재scan). (doc-author 는 기존 문서 **내용**만 입력으로 받음.)
     - **create**: garden 이 spawn 전 target 부재를 기록 → Write **직전** `lstat(target_path)` — **존재/심볼릭이면 fail-closed**(scan~Write 사이 생성됐을 수 있음). "이미 존재 — 재scan 또는 restructure 로 전환?" 승인 요청.
     - 검증 test: target 을 draft~Write 사이 mutate → garden 이 적용 거부함을 증명.
...
1. **원본 비파괴 — 권한 수준 완전 보장 + TOCTOU** `[R1:F1][R2][R3-plan:🔴 create-TOCTOU]`: doc-author 는 **Write·Bash 둘 다 없으므로** 어떤 경로로도(직접 Write, shell redirection/`sed -i`/`tee`) target 문서를 쓸 수 없다. garden 만 승인 후 Write. **Write 직전 §5 ① TOCTOU 재확인 필수 (garden 소유 — doc-author 는 Bash 없어 hash 계산 불가)** — restructure 는 **garden 이 spawn 전 캡처한 baseline `git hash-object` 와 Write 직전 재계산이 일치**하는지, **create 는 garden 이 `lstat` 로 여전히 부재인지** 확인(존재/심볼릭이면 fail-closed). scan~Write 사이 파일 생성/변경(stale 재사용 scan/동시 편집/non-git)으로 인한 silent overwrite 방지. restructure 는 추가로 per-removal 승인 + 미승인 재삽입 통과 후에만 교체.
