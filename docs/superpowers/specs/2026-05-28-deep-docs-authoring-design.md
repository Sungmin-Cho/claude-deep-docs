...
**`[R1:F2][R2]` schema 버전 (D10) — exact 6곳 + validator 일괄 교체**: `envelope.schema.version` 1.0 → **1.1**. exact `=== "1.0"` 검사가 다음 **7개 지점**에 하드코딩돼 있어 **모두 `"1.1"` 로 교체**한다(개정 누락 시 fixture 1.1 이 `validate-envelope:115` 에서 즉시 fail → `verify-fixes:157` fail → 릴리스 차단; R2:N1):
- `agents/doc-scanner.md:291`(envelope contract 명세), `:334`(reuse 가드)
- `skills/deep-docs-workflow/SKILL.md:32`, `:53` (garden/audit 가드)
- `skills/deep-docs/SKILL.md:90`, `:219` (garden/audit 가드)
- `scripts/validate-envelope-emit.js:115` (`if (env.schema?.version !== '1.0') fail(...)`)

legacy `1.0` 아티팩트는 가드 불일치로 **즉시 재-scan**(producer 가 self-emit 하므로 혼용 전환기 없음; gaps[] 부재 케이스 자연 회피). top-level envelope wrapper `schema_version` 은 `"1.0"` **유지**(envelope 구조 불변 — payload schema 만 1.1).
...
- **출력 계약 (구조화 — 확정)** `[R1:F1,C5][R2: codex high default-keep]`: doc-author 는 파일을 쓰지 않고(Write·Bash 없음) **구조화된 result 객체를 반환**한다:
  ```jsonc
  {
    ...
    "removal_candidates": [ { "text": "<제거 후보 원문>", "reason": "<재생성 가능 근거>" }, … ]
  }
  ```
  **garden 의 default-keep 기계적 강제** (codex high — prose 가 아닌 contract 로): garden 은 ① restructure 시 `base_hash` 로 기존 문서가 그 사이 변경되지 않았는지 확인(TOCTOU 방지), ② `removal_candidates` 를 사용자에게 **per-removal 명시 승인** 요청, ③ **미승인 removal 은 `draft_body` 에 재삽입한 뒤** Write — 즉 승인 안 한 고유 콘텐츠는 기계적으로 보존된다(silent omit 불가). `draft_body` 는 메타텍스트(요약/강등사유) 없는 단일 구획이므로 garden 이 그대로 Write.
...
5. **길이 가드 — 목표 vs 차단 분리** `[R1:F3,F11][R2:F3 현실성]`: draft **목표** = CLAUDE ≤100줄 / AGENTS ≤100줄+≤32KiB / ARCH ≤300줄.
   - **100~200줄 구간(CLAUDE)은 authoring 실패가 아니다** — size-warning(audit-only, **비차단**)으로 보고만 된다. doc-author 는 100줄을 *지향*하되 과압축으로 정보를 잃지 않는다(실제 프로젝트 CLAUDE.md 는 흔히 150~200줄; 100줄 hard target 은 비현실적이라 R2 지적).
   - **hard ceiling 만 강제 분할**: CLAUDE 200줄 / AGENTS 32KiB(Codex 런타임 잘림선) 초과 시 doc-author 가 분할/압축 제안.
...
- `validate-envelope-emit.js` **책임 경계 + schema 가드 교체** `[R1:F2][R2:N1]`:
  - **라인 115 교체 (필수)**: `if (env.schema?.version !== '1.0')` → `!== '1.1'`. 이 교체 없이 fixture 를 1.1 로 올리면 validator 가 즉시 fail → `verify-fixes:157` fail → 릴리스 차단(N1). "envelope/identity 검사 기존대로"라는 서술은 **틀렸으므로 삭제** — schema.version 체크는 예외적으로 변경된다.
...
2. **payload `schema.version` 1.0 → 1.1 — exact 7곳 교체** `[R1:F2][R2:N1]`: emit(doc-scanner Step 13) + 가드 6곳(doc-scanner:291,334 / workflow SKILL:32,53 / entry SKILL:90,219) + `validate-envelope-emit.js:115` + fixture 를 모두 `"1.1"` 로. (§4.5/§8 참조 — 누락 시 verify-fixes 차단.)
3. **cross-repo 순서 분리** `[R1:F2][R2:F7]`:
   ...
   - **후속(non-blocking)**: deep-dashboard 는 `summary.total_issues`/`auto_fixable` 숫자만 소비하고 enum strict 검증 안 함(codex 코드 확인) → `total_issues` 의미 보존(D12)으로 **즉시 영향 없음**. dashboard 의 authoring 표시 강화는 후속 이슈.
