...
| 규칙 | 설명 | 자동 수정 안 하는 이유 |
|------|------|----------------------|
| 크기/구성 | CLAUDE.md/AGENTS.md >100, README.md >300, 기타 docs/ >200 | 분리는 구조적 판단(섹션 경계, 외부 참조)이 필요. 분리 제안만 표시 |
...
2. auto-fix 가능 항목만 추출합니다 (죽은 참조, 이동된 경로, 확인된 오래된 예시, 완전 중복). 크기 경고는 분리가 구조적 판단을 필요로 하므로 audit-only 요약에 표시되며 자동 수정되지 않습니다.
...
모든 스캔은 **claude-deep-suite M3 cross-plugin envelope** 으로 wrap된 내구성 아티팩트를 기록합니다 (`claude-deep-suite/docs/envelope-migration.md` 참조):
...
`garden`과 `audit`은 다음 **4가지 조건이 모두** 충족될 때 이 아티팩트를 재사용합니다 (envelope-aware):
- `schema_version === "1.0"` AND `envelope.schema.version === "1.0"`
- `envelope.generated_at` 이 **10분 이내**
- `envelope.git.head` 가 `git rev-parse HEAD` 와 일치 (git 환경)
- `payload.provenance.worktree_hash` 가 재계산값과 일치 (git 환경)

이전 v1.1.0 shape (`schema_version: 2` numeric, root-level `scanned_at`, `provenance.head_sha`) 은 1번 검사에서 자동 fail → 재-scan 트리거. 10분 TTL 자연 invalidation 으로 마이그레이션 도구 불필요.

`worktree_hash`는 추적된 diff + 미추적 파일 목록/내용을 커버합니다 (NUL-safe, 파일별 git-hash-object). `scan-filters/worktree-hash.md` 참조.

git을 사용하지 않는 환경에서는 10분 TTL만 적용됩니다. envelope 은 sentinel 값으로 `git = { "head": "0000000", "branch": "HEAD", "dirty": "unknown" }` 를 emit 합니다.
