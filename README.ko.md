...
`garden`과 `audit`은 다음 두 조건이 모두 충족될 때 이 아티팩트를 재사용합니다:
- 생성된 지 **10분 이내**, 그리고
- 저장된 `head_sha`가 현재 `git rev-parse HEAD`와 일치

git을 사용하지 않는 환경에서는 10분 TTL만 적용됩니다. 두 조건 중 하나라도 충족되지 않으면 진행 전에 스캔이 자동으로 다시 실행됩니다.
